...
class Controller:
    ...
    def check_user_answer(self, answer):
        if answer == "1":
            ...
            try:
                article = self.article_model.get_single_aticle(article_title)
                self.user_interface.show_single_article(article)
            except KeyError:
                self.user_interface.show_incorrect_title_eror(article_title)
            else:
                self.user_interface.show_single_article(article)
            ...
            article_title=self.user_interface.get_user_article()
            try:
                title=self.article_model.remove_article()
            except KeyError:
                self.user_interface.show_incorrect_title_eror(article_title)
            else:
